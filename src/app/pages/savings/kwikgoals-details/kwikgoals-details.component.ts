...
@Component({
  selector: 'app-kwikgoals-details',
  templateUrl: './kwikgoals-details.component.html',
  styleUrls: ['./kwikgoals-details.component.scss']
})
export class KwikgoalsDetailsComponent implements OnInit {
  id = '';
  loading = false;
  isLoading = false;
  savingsWallet:any;
  ...
  fetchSingleSaving(loading = true) {
    ...
    this.service.getSingleKwikGoal(this.id).pipe(takeUntil(this.destroy$)).subscribe({
      next: (data: any) => {
        console.log(data)
        this.savingDetails = data.data;
        this.userDetails = data.data.user;
        this.savingsHistory = data.data.user.most_recent_credit;
