...
export class SingleSavingComponent implements OnInit, OnDestroy, AfterViewInit {
  investment_id = '';
  loading = false;
  isLoading = false;
  ...
  constructor(private service: ProjectService, private fb: FormBuilder, private activatedROute: ActivatedRoute, private alertService: AlertService) {
    this.activatedROute.params.subscribe((param: Params) => {
      this.investment_id = param['investment_id'];
      this.fetchSingleSaving();
    })
  ...
  fetchSingleSaving(loading = true) {
    this.loading = loading;
    this.service.getSingleSaving(this.investment_id).pipe(takeUntil(this.destroy$)).subscribe({
      next: (data: any) => {
        console.log(data)
        this.savingDetails = data.savings;
        this.userDetails = data.savings.user;
        this.savingsHistory = data.savings.savings_history;
        this.loanTransactions = data.savings.transactions;
        this.loading = false;
  ...
  cancelOrRejectSavings(type: string) {
    if (confirm("Are you sure?")) {
      this.loading = true;
      this.service.rejectOrCancelSaving(this.investment_id, type).subscribe({
        next: data => {
          this.alertService.success(data.message);
          this.fetchSingleSaving();
        },
