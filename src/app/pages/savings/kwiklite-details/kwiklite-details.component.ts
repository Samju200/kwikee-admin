...
@Component({
  selector: 'app-kwiklite-details',
  templateUrl: './kwiklite-details.component.html',
  styleUrls: ['./kwiklite-details.component.scss']
})
export class KwikliteDetailsComponent implements OnInit {
  id = '';
  loading = false;
  isLoading = false;
  ...
  fetchSingleKwikLite(loading = true) {
    this.loading = loading;
    this.service.getSingleKwikLite(this.id).pipe(takeUntil(this.destroy$)).subscribe({
      next: (data: any) => {
        console.log(data)
        ...
        this.savingsKwiklite = data.data.user.kwik_lite;
