...
export class SingleLoanComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    this.performUnderwritingModal = new Modal('#performUnderwritingModal', modalOptions);
    ...
    this.transactionsHistoryModal = new Modal('#transactionsHistoryModal', modalOptions)
  }
  ...
  onDocumentInputChange() {
    this.uploadDocumentsForm.get('type')?.valueChanges.subscribe(value => {
      console.log(value)
      if (value == '0') {
        this.uploadDocumentsForm.get('otherDocumentName')?.addValidators(Validators.required)
      } else {
        this.uploadDocumentsForm.get('otherDocumentName')?.removeValidators(Validators.required)
      }
  ...
  fetchUserDetails(loading = true) {
    ...
    this.service.getSingleLoan(this.auth_id).pipe(takeUntil(this.destroy$)).subscribe({
      next: (data: any) => {
        console.log(data)
        ...
        this.underwritingChecklist = data.user.underwritingchecklist;
  ...
  fetchCreditHistory() {
    this.loadingCredit = true;
      this.service.getCreditHistory(this.auth_id).pipe().subscribe({
        next: (data: any) => {
  ...
  performIDCardCheck() {
    if(confirm('Are you sure?')) {
      this.loading = true;
      this.service.performIDCardCheck(this.auth_id).pipe(takeUntil(this.destroy$)).subscribe((data: any) => {
  ...
  performFaceMatch() {
    if (confirm('Are you sure?')) {
      this.loading = true;
      this.service.doFaceMatch(this.auth_id).pipe(takeUntil(this.destroy$)).subscribe((data: any) => {
        this.loading = false
        this.alertService.success(data.message);
        this.fetchUserDetails();
      }, () => this.loading = false)
  ...
  performAntiFraudCheck(objectParam: any) {
    if (confirm('Are you sure?')) {
      ...
      this.loading = true;
      this.service.performAntiFraudCheck(param).pipe(takeUntil(this.destroy$)).subscribe((data: any) => {
        this.alertService.success(data.message);
        this.fetchUserDetails();
        this.loading = false;
      }, () => this.loading = false)
  ...
  performUnderwritingCheck(objectParam: any) {
    if (confirm('Are you sure?')) {
      ...
      this.loading = true;
      this.service.performUnderwritingCheck(param).pipe(takeUntil(this.destroy$)).subscribe((data: any) => {
        this.alertService.success(data.message);
        this.fetchUserDetails();
        this.loading = false;
      }, () =>  this.loading = false)
  ...
  openTransactionHistoryModal(credit_id: string) {
    this.loading = true;
    this.service.fetchTransactionsHistory(credit_id).pipe(takeUntil(this.destroy$)).subscribe((data: any) => {
      this.loading = false;
  ...
  rejectOrCancelLoan() {
    if (this.reasonControl.valid) {
      if (confirm('Are you sure you want to proceed?')) {
        ...
        this.service.rejectOrCancelLoan(rejectData, this.rejectOrCancelAction).pipe(takeUntil(this.destroy$)).subscribe(
          {
            next: data => {
              this.alertService.success(data.message);
              this.fetchUserDetails();
  ...
  get documentPreview() {
    return this.documentFile ? (this.documentFile as File).type.startsWith('image') : null
  }
  ...
  uploadDocuments() {
    if (this.uploadDocumentsForm.valid) {
      ...
      this.service.uploadLoanDocument(formData).pipe(takeUntil(this.destroy$)).subscribe({
        next: data => {
  ...
  linkBankCardOrStatement(linkType: 'card' | 'statement') {
    if (confirm('Are you sure?')) {
      this.service.linkBankCardOrStatement(this.auth_id, linkType).pipe(takeUntil(this.destroy$)).subscribe({
        next: (data: any) => {
          this.alertService.success(data.message);
          this.fetchUserDetails();
        }
  ...
  performUnderwriting() {
    if (this.underwritingForm.valid) {
      ...
      this.isLoading = true;
      this.service.performUnderWritingOrAddRepayment(params, this.repayOrUnderwriting == 'underwriting'? 'approve': 'repayment/add').pipe(takeUntil(this.destroy$)).subscribe({
        next: (data: any) => {
          this.isLoading = false;
  ...
  async openEditLoanModal() {
    ...
    this.editLoanForm.patchValue({ email: this.userDetails.email, telephone: this.userDetails.telephone })
  ...
  fetchAllCompanies() {
    ...
    this.service.getCompanies(params).pipe(takeUntil(this.destroy$)).subscribe({
      ...
      error: () => {
        this.alertService.error('Could not fetch companies. Please refresh and try again');
        this.loadingCompanies = false;
      }
    }
    );
  ...
  editLoan() {
    if (this.editLoanForm.valid) {
      const formValues = this.editLoanForm.value;
      this.isLoading = true;
      this.service.editLoan(formValues).pipe(takeUntil(this.destroy$)).subscribe({
        next: data => {
          this.alertService.success(data.message)
  ...
  moveToPendingUnderwriting() {
    if (confirm('Are you sure?')) {
      this.service.movePendingUnderwriting(this.auth_id, '5').pipe(takeUntil(this.destroy$)).subscribe({
        next: data => {
          this.alertService.success(data.message);
          this.fetchUserDetails();
        }
  ...
  markAsFullyPaid(){
    if (confirm('Are you sure?')) {
      this.loading = true
      this.service.markAsFullyPaid(this.auth_id).pipe(takeUntil(this.destroy$)).subscribe((data: any) => {
        this.loading = false;
        this.alertService.success(data.message);
        this.fetchUserDetails();
      })
  ...
  deleteCardAuthorization(last4: string) {
    if (confirm('Are you sure you want to proceed?')) {
      ...
      this.loading = true
      this.service.deleteCardAuthorization(params).pipe(takeUntil(this.destroy$)).subscribe(
        {
          next: data => {
            this.loading = false;
            this.alertService.success(data.message);
  ...
  requestNewAuthorization(){
    if (confirm('Are you sure?')) {
      this.loading = true
      this.service.requestNewCardAuthorization(this.auth_id).pipe(takeUntil(this.destroy$)).subscribe({
        next: data => {
          this.alertService.success(data.message);
          this.fetchUserDetails();
        }
  ...
  chargeCustomer() {
    if (this.chargeCustomerForm.valid) {
      ...
      this.isLoading = true;
      this.service.chargeLoanCustomer(params).pipe(takeUntil(this.destroy$)).subscribe({
        next: data => {
